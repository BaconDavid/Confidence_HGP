...
def train(loader, model, criterion_cls, criterion_ranking, optimizer, epoch, history, logger, args):
    ...
    for i, (input, target, idx) in enumerate(loader):
        ...
        # compute output
        output = model(input)
        #batch accumulation of 5
        
        y_pred_accumulate.append(output)
        y_true_accumulate.append(target)
        y_idx_accumulate.append(idx)

        if (i+1) % 5 == 0:
            output = torch.cat(y_pred_accumulate, dim=0)
            target = torch.cat(y_true_accumulate, dim=0)
            idx = torch.cat(y_idx_accumulate, dim=0)
            print(output.shape, target.shape, idx.shape,'batch accumulation')
            y_pred_accumulate.clear()
            y_true_accumulate.clear()
            y_idx_accumulate.clear()
        ...
        # max(softmax)
            ...
            # calc target, margin
            rank_target, rank_margin = history.get_target_margin(idx, idx2,device=args.device)
